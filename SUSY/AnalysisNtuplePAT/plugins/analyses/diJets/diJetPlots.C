...
void diJetPlots(std::string mettype="CaloMET", std::string jettype="CaloJets", std::string energy="7TeV")
{
  ...
  filenames[4]  = "LM12";
  filenames[5]  = "LM13";
  filenames[6]  = "SM_Background";

  //filenames[0]  = "LM1";
  //filenames[1]  = "WJets-madgraph";
  //filenames[2]  = "ZJets-madgraph";
  //filenames[3]  = "ZInvisibleJets";
  //filenames[4]  = "TTbarJets-madgraph";  
  //filenames[5]  = "QCD_MadGraph_Pt50toInf";
  //filenames[6]  = "VectorBosons";
  ...
  fillcolor[0] = kBlue;//LM0
  fillcolor[1] = kRed+1;//LM1
  fillcolor[2] = kGreen+1;//LM5
  fillcolor[3] = kOrange+7;//SM
  fillcolor[4] = kBlue+3;//LQ to CMu M300
  fillcolor[5] = kYellow+3;//LQ to CMu M400
  fillcolor[6] = kBlack+3;//LQ to CMu M400
  ...
  for(int tt = 0; tt < NUMVARS; tt++) {
    for (int z = 0; z < NUMHISTS; z++) {
      ...
      gPad->SetLogy(kTRUE);
      hist[0][z][tt]->Draw("p0");
      for (int j = 1; j < NUMFILES; j++) hist[j][z][tt]->Draw("p0same");
      leg->Draw();
      char outputimage[128];
      std::string outhistname = (std::string)histnames[z][tt];
      sprintf(outputimage,"./MET_Analysis/CaloJets/%s.png",outhistname.c_str());
  ...
  for(int ss = 0; ss <2; ss++) {
    for (int y = 0; y < NUMHISTS2; y++) {
      ...
      gPad->SetLogy(kTRUE);
      hist2[0][y][ss]->Draw("p0");
      for (int j = 1; j < NUMFILES; j++) hist2[j][y][ss]->Draw("p0same");
      leg->Draw();
      char outputimage2[128];
      std::string outhistname2 = (std::string)histnames2[y][ss];
      sprintf(outputimage2,"./MET_Analysis/CaloJets/%s.png",outhistname2.c_str());

...
#ifndef optimizeCuts_h
#define optimizeCuts_h
...
class optimizeCuts {
 public :
  ...
  optimizeCuts(TTree *tree=0, bool isData=false, std::string jetPrefix="Calo", std::string metPrefix="CaloTypeI", std::string lepPrefix="", std::string phtPrefix="");
  virtual ~optimizeCuts();
...
#ifdef optimizeCuts_cxx

optimizeCuts::optimizeCuts(TTree *tree, 
	   bool isData, 
...
optimizeCuts::~optimizeCuts() {
  if (!fChain) return;
  delete fChain->GetCurrentFile();
}

Int_t optimizeCuts::GetEntry(Long64_t entry) {
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}

Long64_t optimizeCuts::LoadTree(Long64_t entry) {
...
void optimizeCuts::Init(TTree *tree, TString jetPrefix, TString metPrefix, TString lepPrefix, TString phtPrefix) {
...
Bool_t optimizeCuts::jetID(int index, bool tight) {
...
Bool_t optimizeCuts::muonID(int index, int mode) {
...
Bool_t optimizeCuts::electronID(int index, bool tight ) {
...
Bool_t optimizeCuts::photonID(int index, bool tight) {
...
Double_t optimizeCuts::computeHT(double& minpt, double& maxeta, bool fromRAW) {
...
TLorentzVector optimizeCuts::computeMHT(double& minpt, double& maxeta, bool fromRAW) {
...
Double_t optimizeCuts::computeDPhiStar(TLorentzVector mht, double& minpt, double& maxeta, bool fromRAW) {
...
void optimizeCuts::Show(Long64_t entry) {
...
Bool_t optimizeCuts::goodMuonProbe(int index, int mode, int probeCharge) {
...
Bool_t optimizeCuts::goodMuonTag(int index, int mode) {
  bool result = false;
  bool muonType;
  double relIso = 0;
